...
function projBack = backwardProj_RL_GPU(psf_t,proj,reg_constant)
% INPUT
...
b1 = zeros(r,c,'single');
a1 = zeros(r,c,'single');
projBack=zeros(ra,ca,sb,'single');
...
for z=1:size(psf_t,3)
    ...
    projBack(:,:,z) = real(con1(p1+1:r-p1,p2+1:c-p2)) + reg_constant * eye(ra, ca, 'single');
end
end