...
htf = zeros(proj_r,proj_c,psf_s,angleNum,'single');
for i = 1:angleNum
    htf(:,:,:,i) = backwardProj_RL_GPU(squeeze(psf_t(:,:,:,i)),ones(proj_r,proj_c,'single'), 0);
end
%%%extract struct
...
reg_constant = 0.0002; %0.0003; 0.00001 0.005 regularization constant
...
for iterNow = 1:maxIter
    ...
    if DAO == 1 && iterNow > maxIter_init
        ...
        for angleNow = 1:angleNum
            ...
            HXguess = forwardProj_RL_GPU(squeeze(psfs(:,:,:,angleNow)),XguessDAO);
            clear XguessDAO
    ...
    for upCount = 1:length(upSeq)
        ...
        for angleNow = upAngles
            ...
            %%%forward project to get HXguess
            HXguess = forwardProj_RL_GPU(squeeze(psfs(:,:,:,angleNow)),Xguess);

            %%%calculate error and backward project
            errorEM = projNow./(HXguess + reg_constant1);
